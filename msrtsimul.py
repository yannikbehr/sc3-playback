...
def main():
    ...
    try:
        opts, args = getopt(sys.argv[1:], "cd:s:j:vhm:t:",
                            ["stdout", "delays=", "speed=", "jump=", "test",
                             "verbose", "help", "mode=", "seedlink=", "starttime="])
    except GetoptError:
        usage()
        return 1
    ...
    for flag, arg in opts:
        if flag in ("-c", "--stdout"):
            ...
            verbosity += 1
        elif flag in ("-t", "--starttime"):
            starttime = datetime.datetime.strptime(arg, "%Y-%m-%d %H:%M:%S.%f")
        elif flag == "--test":
            test = True
        else:
            usage()
